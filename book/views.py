...
class BookUpdateView(LoginRequiredMixin,UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        UserActivity.objects.create(created_by=self.request.user.username,
            ...
            target_model=self.model.__name__,
            detail =f"更新 {self.model.__name__} << {current_book.title} >>")
        return super(BookUpdateView, self).post(request, *args, **kwargs)

    def form_valid(self, form):
      title=form.cleaned_data['title']      
      messages.warning(self.request, f"更新 << {title} >> 成功")
...
class BookDeleteView(LoginRequiredMixin,View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        messages.error(request, f"物品 << {delete_book.title} >> 已被移除")
        delete_book.delete()
        UserActivity.objects.create(created_by=self.request.user.username,
            operation_type="danger",
            target_model=model_name,
            detail =f"删除 {model_name} << {delete_book.title} >>")
...
class CategoryCreateView(LoginRequiredMixin,CreateView):
    ...
    #这段代码是一个 Django 视图中的 form_valid 方法。在这个方法中，执行了以下操作：
    # 通过 form.save(commit=False) 创建一个新的 Category 对象 new_cat，但不将其保存到数据库。
    # 调用 new_cat.save() 将新创建的 Category 对象保存到数据库。
    # 调用 send_notification 函数发送通知，通知内容包括了当前用户、新创建的 Category 对象的名称。
    # 使用 logger 记录了操作日志，记录了当前用户创建了哪个 Category 对象。
    # 创建了一个 UserActivity 对象，记录了用户的活动详情，包括了创建了哪个模型以及具体操作的对象名称。
    # 最后调用父类的 form_valid 方法继续处理表单数据。
    # 这段代码主要实现了在用户创建新的 Category 对象时，发送通知、记录日志和用户活动，并最终保存表单数据的逻辑。
    def form_valid(self, form):
...
class CategoryDeleteView(LoginRequiredMixin,View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        messages.error(request, f"分类 << {delete_cat.name} >> 已被删除。")
        delete_cat.delete()
        send_notification(self.request.user,delete_cat,verb=f'Delete Category << {delete_cat.name} >>')
        UserActivity.objects.create(created_by=self.request.user.username,
                            operation_type="danger",
                            target_model=model_name,
                            detail =f"删除 {model_name} << {delete_cat.name} >>")

        logger.info(f'{self.request.user} 删除分类 {delete_cat.name}')
...
class PublisherCreateView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self,form):
        ...
        new_pub.save()
        messages.success(self.request, f"新的提供者 << {new_pub.name} >> 已被添加")
        send_notification(self.request.user,new_pub,verb=f'添加新的 Publisher << {new_pub.name} >>')
        logger.info(f'{self.request.user} created Publisher {new_pub.name}')

        UserActivity.objects.create(created_by=self.request.user.username,
                                    target_model=self.model.__name__,
                                    detail =f"创建 {self.model.__name__} << {new_pub.name} >>")
...
class ActivityListView(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        ...
        all_activities = UserActivity.objects.all().order_by(self.order_field)  # 应用排序参数
...
class MemberCreateView(LoginRequiredMixin,CreateView):
    ...
    def post(self,request, *args, **kwargs):
        ...
        messages.success(request, f"新用户 << {new_member_name} >> 已被添加")
        #在user-activity中创建一条记录
        UserActivity.objects.create(created_by=self.request.user.username,
                                    target_model=self.model.__name__,
                                    detail =f"创建了 {self.model.__name__} << {new_member_name} >>")
    ...
    def form_valid(self, form):
        ...
        send_notification(self.request.user,self.object,f'添加一个新的用户 {self.object.name}')
...
class MemberUpdateView(LoginRequiredMixin,UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        UserActivity.objects.create(created_by=self.request.user.username,
            operation_type="warning",
            target_model=self.model.__name__,
            detail =f"更新 {self.model.__name__} << {current_member.name} >>")
        return super(MemberUpdateView, self).post(request, *args, **kwargs)

    def form_valid(self, form):
        member_name=form.cleaned_data['name']      
        messages.warning(self.request, f"用户 << {member_name} >> 更新成功")
        return super().form_valid(form)
...
class MemberDeleteView(LoginRequiredMixin,View):
    ...
    def get(self,request,*args,**kwargs):
        ...
        messages.error(request, f"用户 << {delete_member.name} >> 已被移除")
        delete_member.delete()
        send_notification(self.request.user,delete_member,f'用户{delete_member.name}已被删除 ')


        UserActivity.objects.create(created_by=self.request.user.username,
                    operation_type="danger",
                    target_model=model_name,
                    detail =f"删除了 {model_name} << {delete_member.name} >>")
...
class BorrowRecordCreateView(LoginRequiredMixin,CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(self.request, f" '{borrower_name}' 租借了 <<{book_name}>>")
        UserActivity.objects.create(created_by=self.request.user.username,
                                    target_model=self.model.__name__,
...
class yonghu(SuperUserRequiredMixin,View):
    def get(self):
     return redirect('notice_list')